...
@section('addCss')
    ...
    <style>
        ...
        /* Animation - Improved untuk prevent flash */
        .fade-in {
        ...
        $(document).ready(function () {
            ...
            var table = $("#data-table").DataTable({
                ...
                "language": {
                    ...
                    "infoEmpty": "Tidak ada buku tersedia",
                    "infoFiltered": "(difilter dari _MAX_ total buku)",
                    "paginate": {
                        "first": "Pertama",
                        "last": "Terakhir",
                        "next": "Selanjutnya",
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <h1 class="m-0">Koleksi <span class="text-gradient">Buku</span></h1>
                <p class="subtitle">Jelajahi koleksi buku perpustakaan digital</p>
            </div>
        </div>
        ...
        <section class="content">
            <div class="container-fluid">
                ...
                <div class="filter-section">
                    <form method="GET" action="{{ route('user.daftarBuku') }}" class="filter-form">
                            ...
                            <select class="form-control" id="kategori" name="kategori">
                                ...
                                @if(isset($kategori))
                                    @foreach($kategori as $item)
                                        <option value="{{ $item->id }}" {{ ($kategori_filter ?? '') == $item->id ? 'selected' : '' }}>
                                            {{ $item->nama_kategori }}
                                        </option>
                                    @endforeach
                                @endif
                        ...
                        <div class="filter-group">
                            ...
                            <a href="{{ route('user.daftarBuku') }}" class="reset-button ml-2">
                    ...
                    <div class="table-header">
                        ...
                        <p class="table-header-subtitle">
                            @if(($search ?? '') || ($kategori_filter ?? ''))
                                ...
                                @if($kategori_filter ?? '')
                                    kategori
                                    "<strong>{{ isset($kategori) ? ($kategori->find($kategori_filter)->nama_kategori ?? 'Tidak Ditemukan') : 'Tidak Ditemukan' }}</strong>"
                                @endif
                                - Ditemukan {{ isset($buku) ? $buku->total() : 0 }} buku
                            @else
                                Menampilkan semua koleksi buku perpustakaan
                            @endif
                        </p>
                    </div>
                            ...
                            <table class="table table-hover" id="data-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width: 200px;">Informasi Buku</th>
                                    ...
                                    @if(isset($buku) && $buku->count() > 0)
                                        @foreach ($buku as $item)
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    @if($item->gambar)
                                                        <img src="{{ asset('storage/buku/' . $item->gambar) }}"
                                                            alt="Gambar {{ $item->judul }}" class="book-image"
                                                            title="{{ $item->judul }}"
                                                ...
                                                <td>
                                                    <div class="book-title">{{ $item->judul ?? 'Tanpa Judul' }}</div>
                                                    @if($item->penulis)
                                                        <div class="book-author">oleh {{ $item->penulis }}</div>
                                                    @endif
                                                    @if($item->isbn)
                            ...
                            <div class="row align-items-center">
                                <div class="col-sm-6">
                                    <small>
                                        ...
                                        @if(($search ?? '') || ($kategori_filter ?? ''))
                                            Menampilkan <strong>{{ $buku->count() }}</strong> dari
                                            <strong>{{ $buku->total() }}</strong> buku
                                        @else
                                            Total <strong>{{ $buku->total() }}</strong> buku tersedia
                                        @endif
                                    </small>
                                </div>
